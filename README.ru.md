...
- `observability::ObservabilitySink`: необязательный синхронный
  неавторитетный sink структурированных событий query, cache, Fake IP, hook,
  upstream, timeout и error. События неизменяемы и ограничены; callback sink не
  может менять маршрутизацию, ответы, retry или состояние кэша.

Запланировано для стадии 0.6 (см. [ROADMAP.ru.md](ROADMAP.ru.md)):
...
- Воспроизводимые проверки пакетов/docs.rs и идемпотентная release automation
