...
Реализовано (стадии 0.1-0.2, плюс Track A, Track B и Track C стадии 0.3):
...
- Failover между апстримами и входящий слушатель сервера (остаток стадии 0.3)
...
Реализация стадий 0.1-0.2 и Track A/Track B/Track C стадии 0.3 из [архитектуры](ARCHITECTURE.ru.md)
...
UDP/TCP/DoT/DoH/DoQ апстрим-транспортом, пригодные для самостоятельного
использования уже сегодня, до появления failover между апстримами и
...
| Failover между апстримами + слушатель сервера | план (0.3) |
...
4. **Стадия 0.3: Апстрим-транспорты и слушатель сервера** *(Track A, Track B и Track C завершены)* — стабилизированный трейт апстрим-бэкенда, базовый UDP/TCP, DoT/DoH/DoQ за Cargo-фичами `dot`/`doh`/`doq`; fallback/failover между апстримами и входящий слушатель сервера UDP/TCP/DoT/DoH/DoQ ещё впереди.
