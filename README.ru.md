...
Реализовано (стадии 0.1-0.2, плюс Track A и Track B стадии 0.3):
...
- Апстрим-бэкенд DoQ за Cargo-фичей, failover между апстримами и входящий слушатель сервера (остаток стадии 0.3)
...
Реализация стадий 0.1-0.2 и Track A/Track B стадии 0.3 из [архитектуры](ARCHITECTURE.ru.md)
...
доменный/зональный матчер и in-process резолвер с реальным
UDP/TCP/DoT/DoH апстрим-транспортом, пригодные для самостоятельного
использования уже сегодня, до появления DoQ, failover между апстримами и
серверного кода. Fake IP пока нет — см. не-цели стадии 0.2 в
...
| Апстрим DoQ + failover + слушатель сервера | план (0.3) |
...
4. **Стадия 0.3: Апстрим-транспорты и слушатель сервера** *(Track A и Track B завершены)* — стабилизированный трейт апстрим-бэкенда, базовый UDP/TCP, DoT/DoH за Cargo-фичами `dot`/`doh`; DoQ за Cargo-фичей, fallback/failover между апстримами и входящий слушатель сервера UDP/TCP/DoT/DoH/DoQ ещё впереди.
