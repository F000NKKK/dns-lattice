...
Реализовано (стадии 0.1-0.2, плюс Track A, Track B, Track C, Track D и Track E — UDP/TCP/DoT/DoQ — стадии 0.3):
...
- Входящий слушатель сервера DoH (остаток стадии 0.3)
...
Track E (UDP/TCP/DoT/DoQ) стадии 0.3 из [архитектуры](ARCHITECTURE.ru.md)
...
также встраиваемый входящий слушатель DNS-сервера UDP/TCP/DoT/DoQ — всё
пригодно для самостоятельного использования уже сегодня, до появления
входящего слушателя DoH. Fake IP пока нет — см. не-цели стадии 0.2 в
...
| Входящий слушатель сервера DoH | план (0.3) |
...
4. **Стадия 0.3: Апстрим-транспорты и слушатель сервера** *(Track A, Track B, Track C, Track D и Track E — UDP/TCP/DoT/DoQ — завершены)* — стабилизированный трейт апстрим-бэкенда, базовый UDP/TCP, DoT/DoH/DoQ за Cargo-фичами `dot`/`doh`/`doq`, fallback/failover между апстримами внутри группы и встраиваемый входящий слушатель сервера UDP/TCP/DoT/DoQ (`Server`/`ServerBuilder`); входящий слушатель сервера DoH ещё впереди.
