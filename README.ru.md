...
Реализовано (стадии 0.1-0.2, плюс Track A, Track B, Track C и Track D стадии 0.3):
...
- Входящий слушатель сервера (остаток стадии 0.3)
...
Реализация стадий 0.1-0.2 и Track A/Track B/Track C/Track D стадии 0.3 из [архитектуры](ARCHITECTURE.ru.md)
...
UDP/TCP/DoT/DoH/DoQ апстрим-транспортом и failover по бэкендам группы,
пригодные для самостоятельного использования уже сегодня, до появления
...
4. **Стадия 0.3: Апстрим-транспорты и слушатель сервера** *(Track A, Track B, Track C и Track D завершены)* — стабилизированный трейт апстрим-бэкенда, базовый UDP/TCP, DoT/DoH/DoQ за Cargo-фичами `dot`/`doh`/`doq`, и fallback/failover между апстримами внутри группы; входящий слушатель сервера UDP/TCP/DoT/DoH/DoQ ещё впереди.
