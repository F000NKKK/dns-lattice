...
- In-process `Resolver`: создание из `SplitDnsPolicy`, маршрутизация одного запроса к группе апстримов и резолвинг через зарегистрированный бэкенд, с кэшем ответов в памяти с учётом TTL и негативным кэшированием по RFC 2308

Запланировано (см. [ROADMAP.ru.md](ROADMAP.ru.md)):

- Апстрим-бэкенды UDP/TCP/DoT/DoH/DoQ и входящий слушатель сервера (стадия 0.3)
...
Реализация стадий 0.1-0.2 из [архитектуры](ARCHITECTURE.ru.md) покрыта
...
- модуль `engine` фасада `dns-lattice` (`Resolver`, `ResolverBuilder`): создание/резолвинг in-process, статическая split-DNS маршрутизация и кэш ответов в памяти с TTL и негативным кэшированием — реального сетевого транспорта пока нет

Это даёт полностью протестированную модель DNS-сообщений, детерминированный
доменный/зональный матчер и in-process резолвер, пригодные для
самостоятельного использования уже сегодня, до появления реального
транспорта или серверного кода. Реального сетевого ввода-вывода или Fake IP
пока нет — см. не-цели стадии 0.2 в [ROADMAP.ru.md](ROADMAP.ru.md).
...
| Апстрим UDP/TCP/DoT/DoH/DoQ + слушатель сервера | план (0.3) |
