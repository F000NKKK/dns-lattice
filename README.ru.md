...
Реализовано (стадии 0.1-0.2, плюс Track A, Track B, Track C, Track D и базовый UDP/TCP Track E стадии 0.3):
...
- Входящие слушатели сервера DoT/DoH/DoQ (остаток стадии 0.3)
...
UDP/TCP Track E стадии 0.3 из [архитектуры](ARCHITECTURE.ru.md)
...
также встраиваемый входящий слушатель DNS-сервера UDP/TCP — всё пригодно
для самостоятельного использования уже сегодня, до появления входящих
слушателей DoT/DoH/DoQ. Fake IP пока нет — см. не-цели стадии 0.2 в
...
| Входящие слушатели сервера DoT/DoH/DoQ | план (0.3) |
...
4. **Стадия 0.3: Апстрим-транспорты и слушатель сервера** *(Track A, Track B, Track C, Track D и базовый UDP/TCP Track E завершены)* — стабилизированный трейт апстрим-бэкенда, базовый UDP/TCP, DoT/DoH/DoQ за Cargo-фичами `dot`/`doh`/`doq`, fallback/failover между апстримами внутри группы и встраиваемый входящий слушатель сервера UDP/TCP (`Server`/`ServerBuilder`); входящие слушатели сервера DoT/DoH/DoQ ещё впереди.
