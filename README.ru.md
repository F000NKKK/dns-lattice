...
> **Статус:** Стадии 0.1-0.2 завершены; стадия 0.3 реализовала модель DNS-сообщений, доменный/
...
Реализовано (стадии 0.1-0.2, плюс стадия 0.3 на финальной верификации):
...
- Cargo-фича `doh` модуля `server` фасада `dns-lattice` (`ServerBuilder::doh_addr`): входящий слушатель DNS-over-HTTPS, сквозным образом протестированный с локально сгенерированным самоподписанным сертификатом для согласованных через ALPN HTTP/1.1 и HTTP/2, включая GET и POST в обоих протоколах
...
4. **Стадия 0.3: Апстрим-транспорты и слушатель сервера** *(active: финальная верификация)* — стабилизированный трейт апстрим-бэкенда, базовый UDP/TCP, DoT/DoH/DoQ за Cargo-фичами `dot`/`doh`/`doq`, fallback/failover между апстримами внутри группы и встраиваемый входящий слушатель сервера UDP/TCP/DoT/DoH/DoQ (`Server`/`ServerBuilder`).
