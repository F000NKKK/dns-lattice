...
> фасад `dns-lattice`. Разработка `0.4` добавляет opt-in синтез ответов Fake
> IP через резолвер и каждый серверный транспорт. API остаётся pre-1.0; см.
> раздел «Текущий статус» ниже.
...
резолвера, которым пользуется каждый транспорт `Server`.
...
| Пул Fake IP и opt-in синтез через resolver/server | ✅ (разработка 0.4) |
...
5. **Стадия 0.4: Fake IP** *(active)* — детерминированное выделение
