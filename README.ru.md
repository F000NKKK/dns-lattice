...
> **Статус:** Опубликован `0.4.0`. Он реализует модель DNS-сообщений, доменный/
...
> реализует hook pipeline стадии 0.5; он ещё не входит в опубликованный
> `0.4.0`. API остаётся pre-1.0; см. раздел «Текущий статус» ниже.
...
Реализовано (опубликовано по `0.4.0`):
...
Реализовано в `main` для стадии 0.5:
...
состояние разработки также поддерживает opt-in синтез Fake IP в пути запроса
резолвера, которым пользуется каждый транспорт `Server`, и необязательный
hook pipeline стадии 0.5 для обычных запросов. Хук вызывается после выбора
статического кандидата и до кэша с областью маршрута; он не может изменить
terminal обработку Fake IP или получить права на побочные действия ОС/сети.
...
| Пул Fake IP и opt-in синтез через resolver/server | ✅ (0.4.0) |
| Динамические хуки маршрутизации | active в `main` (0.5) |
...
6. **Стадия 0.5: Динамические хуки маршрутизации** *(реализация завершена; ожидается release CI)* — один
