...
> **Статус:** Стадия 0.1 (базовая модель) реализовала модель DNS-сообщений,
> доменный/зональный матчер и типы политик split-DNS в трёх крейтах —
> `dns-lattice-core`, `dns-lattice-model` и фасад `dns-lattice`. Версии
> `0.1.0` опубликованы, чтобы зарезервировать имена крейтов на crates.io;
> стабильного публичного API пока нет. См. раздел «Текущий статус» ниже.
...
- Движок резолвера, кэш ответов в памяти, статическая split-DNS маршрутизация (стадия 0.2)
...
Реализация стадии 0.1 из [архитектуры](ARCHITECTURE.ru.md) покрыта 37
...
- фасад `dns-lattice`, реэкспортирующий всё перечисленное выше

Это даёт полностью протестированную модель DNS-сообщений и детерминированный
доменный/зональный матчер, пригодные для самостоятельного использования уже
сегодня, до появления транспорта или серверного кода. Сетевого ввода-вывода,
кэша или Fake IP пока нет — см. не-цели стадии 0.1 в [ROADMAP.ru.md](ROADMAP.ru.md).
...
| Движок резолвера / кэш ответов | план (0.2) |
...
3. **Стадия 0.2: Движок резолвера и статический split DNS** — точка входа резолвера (создание-резолвинг-остановка), статическая split-DNS маршрутизация, кэш ответов в памяти с негативным кэшированием, фейковый in-process апстрим для детерминированных тестов.
