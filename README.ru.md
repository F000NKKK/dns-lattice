...
Реализовано (стадии 0.1-0.2, плюс стадия 0.3 полностью — Track A, Track B, Track C, Track D и Track E):
...
- Входящий слушатель DNS-over-HTTPS (DoH, RFC 8484) за Cargo-фичей `doh`: `ServerBuilder::doh_addr` принимает TLS-сессию на каждом соединении через `tokio_rustls::TlsAcceptor`, затем обслуживает HTTP/1.1 через сервер-билдер `hyper_util` с автоопределением протокола, разбирая запросы GET (`?dns=` в base64url) и POST (тело `application/dns-message`) по RFC 8484 и отвечая резолвнутым сообщением как телом `application/dns-message` (HTTP 200, включая rcode `ServFail`) либо статусом HTTP 400/404 для запроса, не доходящего до уровня DNS
...
Реализация стадий 0.1-0.2 и стадии 0.3 полностью (Track A/Track B/Track
...
- Cargo-фича `doh` модуля `server` фасада `dns-lattice` (`ServerBuilder::doh_addr`): входящий слушатель DNS-over-HTTPS, протестированный против вручную собранного HTTP/1.1-клиента поверх TLS с локально сгенерированным самоподписанным сертификатом, покрывающий резолвинг GET и POST, синтез `Rcode::ServFail` при ошибке резолвера, статус HTTP 400 при недекодируемом параметре `dns` в GET-запросе и статус HTTP 404 при запросе по неверному пути
...
всё пригодно для самостоятельного использования уже сегодня. Fake IP пока
нет — см. не-цели стадии 0.2 в [ROADMAP.ru.md](ROADMAP.ru.md).
...
4. **Стадия 0.3: Апстрим-транспорты и слушатель сервера** *(завершена — Track A, Track B, Track C, Track D и Track E)* — стабилизированный трейт апстрим-бэкенда, базовый UDP/TCP, DoT/DoH/DoQ за Cargo-фичами `dot`/`doh`/`doq`, fallback/failover между апстримами внутри группы и встраиваемый входящий слушатель сервера UDP/TCP/DoT/DoH/DoQ (`Server`/`ServerBuilder`).
