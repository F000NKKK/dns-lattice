...
CRATE_PREFIX="dns-lattice-"
CRATES_IO_USER_AGENT="release.sh (dns-lattice; https://github.com/F000NKKK/dns-lattice)"
...
# сработает (см. CHANGELOG: baг с dns-lattice-backend-darwin).
update_ref() {
...
resolve_crate_action() {
    ...
    # dns-lattice-backend-darwin. --patch по-прежнему не проверяется:
