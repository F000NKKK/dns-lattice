...
caller-owned process-local mapping snapshots. Development on `main` adds the
stage 0.5 route-selection hook pipeline before release validation: selection
is before route-scoped caching, while timeout/retry/cancellation cleanup and
external side effects stay in the hook implementation or composing
application. No public API is stable yet. The most valuable contributions right
now are:
