...
- Test Linux, Windows, and macOS behavior separately once platform-specific
  ...
  parity. DNS Lattice's core is cross-platform by design (`ARCHITECTURE.md`
  "Cross-platform first"), so most tests should already be fully portable —
  treat any test that isn't as a signal the code under test may have
  accidentally grown a platform assumption.
