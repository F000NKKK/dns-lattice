...
DNS Lattice tracks all roadmap work in the YouTrack project `DL`
(https://hush.youtrack.cloud/projects/DL), via the `youtrack` MCP tools
(`mcp__youtrack__*`). This replaces the former file-based `.ai/<task-name>/`
workflow (`plan.md`, `AUDIT.md`, `adr/`), which has been retired.

## Issue hierarchy

- `Epic` — one roadmap stage (0.3, 0.4, 0.5, ...). Created once per stage.
- `User Story` — a bounded track or slice inside a stage (e.g. "Track A —
  UDP/TCP upstream backends"), linked as a subtask of its Epic. **A User
  Story must always own at least one child Task.** This is an inviolable
  rule, not a guideline: never leave a Story without Tasks, even
  temporarily, and never advance a Story's `Stage` past `Backlog` while it
  has zero child Tasks. If a Story's scope isn't decomposed yet, the very
  first thing owed to it is a scoping Task (see "Decomposing a stage"
  below) — create that Task in the same turn the Story itself is created,
  don't leave the Story bare pending a "later" pass.
- `Task` — one bounded checkbox-equivalent unit of work: model, engine,
  server, upstream, fakeip, hooks, facade, test, CI, documentation, or
  packaging subtask. Linked as a subtask of its Story (or directly of its
  Epic for small stages).
...
use `mcp__youtrack__link_issues` for non-parent relations. Confirm this
instance's actual link-type set from the tool's own error response rather
than assuming it matches another project — do not assume a `blocked by` type
exists; use `depends on` (the issue passed as `targetIssueId` depends on the
one passed as `issueToLinkId`) to express "A is blocked by B" unless the
schema proves otherwise.

## Custom fields

Every issue in `DL` carries:
...
- `Platform` (multi-value) — `Linux` | `Windows` | `Darwin` |
  `Cross-platform`. DNS Lattice's core is cross-platform by design (no
  `cfg`-gated core logic, per `ARCHITECTURE.md`'s "Cross-platform first"
  goal) — most Tasks should be `Cross-platform`; use a specific OS only for
  backend I/O (socket/TLS/QUIC transport) that genuinely varies by platform
  capability. Set explicitly on every Task/Bug; never leave it unset.
- `Sprint DL` — the roadmap-stage label (`0.3`, `0.4`, `0.5`, ...). Note the
  field's exact name is `Sprint DL`, not `Sprint` (distinct from the `NL`
  project's field name — do not assume they match across Lattice
  repositories). The user creates and manages Sprint entities directly in
  YouTrack (Board → Sprints); agents never create a Sprint value, only set
  the field on an issue. Set `Sprint DL` on every issue to the stage it
  belongs to — this is orthogonal to `Stage` (workflow state) and to the
  Epic hierarchy: it is what the Agile boards use to pick which issues are
  "in view" for a given roadmap stage. Never leave it unset on a new issue;
  if the target stage has no Sprint value yet, ask the user to create it
...
- `Done` — the primary agent (or the reviewer directly, when it was able to
  ...
  and closing one does not close the other. Whichever agent performs the
  review — subagent or primary — must set `Done` on every Task the review
  covers, not only its own.
...
- `Sprint DL` — set at issue creation by whichever role creates it, to an
  existing Sprint value only (see above — never invented).
...
mcp__youtrack__search_issues
  query: "project: DL Sprint DL: {0.3} Type: Task Stage: Backlog"
```
...
- `project: DL Type: Task Stage: Backlog Sprint DL: {0.3}` — unstarted work
  in a stage.
- `project: DL Role: Implementer Stage: Develop` — Tasks currently mid-
  implementation.
- `project: DL Type: Bug Stage: -Done` — open bugs.
- `project: DL Platform: Windows` — Windows-specific issues.
...
Agile board(s) on `DL` are driven by the `Stage` field for columns; check
what's configured before assuming a specific swimlane grouping exists (the
`NL` project uses one Epic-swimlane and one Story-swimlane board — `DL` may
not have both yet). Boards are UI-configured (columns/swimlanes/filters);
agents cannot create or reconfigure boards via the available MCP tools —
flag it to the user if a board needs a setup change instead of attempting a
workaround via issue links.
...
Article in the `DL` knowledge base (via `mcp__youtrack__create_article`),
filed under the "Architecture Decision Records (ADR Index)" parent article
(`DL-A-1`). Use the same Context / Decision / Alternatives considered /
...
(e.g. `DL-A-2`) in the issue description or a comment — YouTrack does not
...
the count. Before filing a new ADR, read `DL-A-1` (`mcp__youtrack__get_article`,
...
stage's own ADR count — `net-lattice`'s `NL` project hit exactly this bug
once (a stage-local count produced a numbering collision); do not repeat it
here.
...
- Before filing any new Story/Task under an Epic, search for existing
  ...
  tree; checking first is one search call, skipping it risks redoing
  verified work under a second set of IDs (this exact failure mode has
  already occurred once on the sibling `net-lattice`/`NL` project).
- Start from one Epic (a roadmap stage) and decompose it into the applicable
  model, engine, server, upstream, fakeip, hooks, facade, test, CI,
  documentation, and packaging Tasks, same as the retired `audit.md`
  decomposition rule.
...
`.ai/0.0-audit-roadmap-architecture/`, `.ai/0.1-core-model/`,
`.ai/0.2-resolver-engine/`) — check comments on the DL Stage 0.3 Epic and
the ADR Index (`DL-A-1`) before assuming information is lost, then flag it
for cleanup rather than silently recreating file-based tracking.
