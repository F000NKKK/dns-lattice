# DNS Lattice Claude Code configuration
...
plans, evidence, and decisions live in the YouTrack project `DL`
...
2. Identify the active Epic (roadmap stage) and Task/Story via
   ...
   comments, and any linked ADR Articles under `DL-A-1` before editing
   anything.
...
## Known field-schema gap

The `DL` project's `Role` and `Platform` custom fields exist but currently
have no populated value bundle (creating an issue with `Role: "Primary"` or
`Platform: ["Cross-platform"]` fails with "has no value(s)"). Until the user
populates these bundles in YouTrack (Role: `Researcher`/`Architect`/
`Implementer`/`Reviewer`/`Primary`; Platform: `Linux`/`Windows`/`Darwin`/
`Cross-platform`), omit these two fields when creating issues rather than
letting creation fail — see `rules/youtrack.md`.
