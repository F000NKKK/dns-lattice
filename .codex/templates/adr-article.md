...
`POST /api/articles?fields=idReadable`, parented under `DL-A-1` (the ADR
...
Reference the resulting Article ID (e.g. `DL-A-12`) as explicit text from
the governing Task/Story's description or a comment. Move `Status` from
`proposed` to `accepted` only once the Verification section's evidence is
actually satisfied, mirroring how `rules/youtrack.md` gates `Stage: Done`.
