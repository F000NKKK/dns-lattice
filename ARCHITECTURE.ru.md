...
- `fakeip`: вызываемую приложением конфигурацию пула, типы поиска/обратного
  поиска, TTL и in-memory snapshot/restore. Он не изменяет неявно ответы
  resolver/server, не синтезирует PTR и не сохраняет отображения
  долговременно.
