...
Статус: черновик. Стадия 0.1 (базовая модель) уже реализовала крейты
`dns-lattice-core` и `dns-lattice-model` ниже; последующие стадии
реализуют остальную целевую форму постепенно. Обновляйте документ при
каждом изменении публичного контракта; внутренние записи проектных решений
ведутся отдельно и не являются частью этого документа.
...
dns-lattice          Фасадный крейт: реэкспортирует model/core (а позже server/engine/upstream/fakeip/hooks) как стабильную публичную поверхность крейта
```
...
фиксируется в ADR. `dns-lattice-core` и `dns-lattice-model` — единственные
выделенные пока крейты (`ADR-0004`, стадия 0.1); дальнейшее разделение
...
Крейт `dns-lattice` — единственное место, откуда импортируют внешние
крейты. Он как минимум реэкспортирует:
