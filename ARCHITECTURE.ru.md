...
`1.0.0` на crates.io; публичный API теперь следует обычной SemVer-дисциплине
внутри линейки `1.x`.

Этот документ описывает архитектуру на границе стабильного релиза 1.0.0.
Обновляйте его при каждом будущем минорном/мажорном релизе, меняющем
публичный контракт.
