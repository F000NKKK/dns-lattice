...
Статус: черновик. Стадии 0.1-0.2 уже реализовали крейты
`dns-lattice-core` и `dns-lattice-model`, резолвер/кэш, апстрим-транспорты
и входящие серверные слушатели ниже; последующие стадии реализуют остальную
целевую форму постепенно. Обновляйте документ при каждом изменении
публичного контракта; внутренние записи проектных решений ведутся отдельно
и не являются частью этого документа.
