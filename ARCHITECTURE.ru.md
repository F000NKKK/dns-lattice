...
и входящие серверные слушатели. Стадия 0.4 активна и содержит pool-only
реализацию Fake IP; последующие стадии реализуют остальную целевую форму
постепенно. Обновляйте документ при каждом изменении публичного контракта.
...
dns-lattice          Фасадный крейт: реэкспортирует model/core/engine/server/upstream (а позже fakeip/hooks) как стабильную публичную поверхность крейта
```
...
├── fakeip         Вызываемый приложением пул Fake IP: выделение, обратный поиск, LRU-вытеснение
...
flowchart LR
    ...
    Host -->|явное выделение / обратный поиск| FakeIP[Пул Fake IP]
    Answer --> Listener
    Listener --> Client
```
...
- `fakeip`: вызываемую приложением конфигурацию пула и типы поиска/обратного
  поиска. Он не изменяет неявно ответы resolver/server, не синтезирует PTR,
  не истекает/не сохраняет отображения и не интегрируется с туннелем.
